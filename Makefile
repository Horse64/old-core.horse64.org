...
ifeq ($(PLATFORM),linux)
ifeq ($(CROSSCOMPILEHOST),)
	cd "$(SDLPATH)" && bash ./autogen.sh && ./configure --disable-video-opengles1 --disable-video-vulkan --disable-video-wayland --enable-sse3 --disable-oss --disable-jack --enable-static --disable-shared --enable-ssemath
else
	cd "$(SDLPATH)" && bash ./autogen.sh && ./configure --host="$(CROSSCOMPILEHOST)" --disable-video-opengles1 --disable-video-vulkan --disable-video-wayland --disable-oss --disable-jack --enable-static --disable-shared
endif
else
ifeq ($(PLATFORM),windows)
